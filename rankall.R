rankall <- function(outcome, num = "best") {
  ...
  ## get all & unique state name from the data frame  
  states_vec <- sort(unique(df$State))
  ...
  ## 30-day death rate
  rankall_df <- df[0,c("Hospital.Name",'State')]
  ...
  for (state in states_vec){
    ...
    ## select the rows with state name and desired outcome type
    interim_df <- df[df$State==state,c("Hospital.Name", 'State',outcome)]
    ...
    best_hospital <- data.frame(State=state, Hospital.Name=NA)
    #interim_df[0,c("Hospital.Name", 'State')]
  
    ## create the rank_vector
    rank_vec <- rank(interim_df[[outcome]],na.last=FALSE, ties.method = "first")
  
    if ( num  == 'best') {
      best_hospital <- interim_df[rank_vec[rank_vec==min(rank_vec)],c("Hospital.Name", 'State')]
    
    }
  
    if (num == 'worst') {
      best_hospital<- interim_df[rank_vec[rank_vec==max(rank_vec)],c("Hospital.Name", 'State')]
    }
  
    if (num %in% rank_vec ) {
      best_hospital<- interim_df[rank_vec[rank_vec==num],c("Hospital.Name", 'State')]
    } 
    rankall_df <- rbind(rankall_df,best_hospital)
  }
  col_name<- c('state','hospital')
  names(rankall_df)<- col_name
  rankall_df[,c(2,1)]
}
